...
(define force (verb (list 'force 'pull) "force" #t))
(record-element! 'force force)
...
(define door
  (thing 'door
         ...
         (list
          ...
          (cons force
                (lambda ()
                  "You better find a key, this thing won't open easily"))
          )))
...
(define (using-verb cmd verbs success-k)
  (ormap (lambda (vrb)                        ; para cada verbo em contexto
           (and (memq cmd (verb-aliases vrb)) ; se CMD for membro da lista de alias do verbo
                (success-k vrb)))             ; aplica função success-k ao verbo
         verbs))     ; note que ormap retorna falso se não achar o comando nos verbos
